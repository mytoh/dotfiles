...
if which gosh >&-
  function yotsuba
  command gosh yotsuba-get.scm
  end
  function futaba
  command gosh futaba-get.scm
  end
  function spc2ubar
  command gosh space2underbar.scm
  end
  function ea
  command gosh extattr.scm
  end
  function unpack
  command gosh unpack.scm
  end
  if test -e $GAUCHE_LOAD_PATH/ls.scm
    function ls
 command gosh ls.scm -d
...
switch (uname) 
 ...
 sudo portsnap fetch update
    ...
    function pcheck
    sudo portmaster -PBidav; and sudo portaudit -Fdav; and sudo portmaster -y --clean-packages --clean-distfiles --check-depends
    end
    function pfetch
